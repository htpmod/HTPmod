...
## pca biplot
pca.biplot <- function(pca.scores, pca.loadings, R2, pcs, sp=17, lp=1, sc="blue", lc="red", 
	...
	panel <- function(x, y, i, j, ext.loadings=NULL, ext.scores=NULL, lc='red', ## col='blue', 
		...
		if(!missing(ext.loadings)){
			segments(0, 0, ext.loadings[, i], ext.loadings[, j], col=lc, lwd=0.5)
			if(loading.label){
	...
	my.pairs(pca.scores, labels=pc.labels, xlim=c(-1, 1), ylim=c(-1, 1), 
