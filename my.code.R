...
hist.plot <- function(data, col="grey", ...){
	avg <- unlist(lapply(data, function(x) mean(x, na.rm=TRUE)))
	std <- unlist(lapply(data, function(x) sqrt(var(x, na.rm=TRUE)/length(x))))
	barx <- barplot(avg, beside=TRUE, las=2, xpd=FALSE, border=NA, col=col, ...)
	text(barx, avg+abs(par('usr')[4]-par('usr')[3])*0.02, labels=format(avg, dig=3))
	error.bar(barx, avg, std, col=col, ...)
	invisible(barx)
}
